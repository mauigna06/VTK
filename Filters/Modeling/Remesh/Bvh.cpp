...
void Bvh::build(Mesh* meshPtr0, BoundingBox& boundingBox)
{
    ...
    const int faceCount = static_cast<int>(meshPtr->faces.size());
    ...
    while (!stack.empty())
    {
        ...
        BoundingBox currentBox(meshPtr->faces[startId].boundingBox());
        BoundingBox boundingCentroid(meshPtr->faces[startId].centroid());
        for (int i = startId + 1; i < endId; i++)
        {
            currentBox.expandToInclude(meshPtr->faces[i].boundingBox());
            boundingCentroid.expandToInclude(meshPtr->faces[i].centroid());
        }
        node.boundingBox = currentBox;
        ...
        double splitCoord = 0.5 *
            (boundingCentroid.min[maxDimension] + boundingCentroid.max[maxDimension]);
